...
//     try {
//       const response = await biometricClient.post('/api/enroll', { student_id: studentId, image: imageBase64 });
//       const { embedding_id } = response.data.data;
//       logger.info(`Face enrolled for student ${studentId}`);
//       return { message: 'Face enrolled successfully', embedding_id };
//     } catch (error) {
//       if (axios.isAxiosError(error)) throw new AppError(error.response?.data?.message || 'Face enrollment failed', 502);
//       throw error;
//     }
//   }
...
//     if (!student) throw new AppError('Student not found', 404);
//     try {
//       const response = await biometricClient.post('/api/enroll/multi', { student_id: studentId, images });
...
//     try {
//       const response = await biometricClient.post('/api/recognize', { image: imageBase64, course_id: courseId });
//       const result = response.data.data;
//       if (!result.matched) return { matched: false, message: result.message };
//       return await this._markAttendance(result, courseId);
//     } catch (error) {
//       if (axios.isAxiosError(error)) throw new AppError(error.response?.data?.message || 'Face recognition failed', 502);
//       throw error;
//     }
//   }
...
//     if (!course) throw new AppError('Course not found', 404);
//     try {
//       const response = await biometricClient.post('/api/recognize/confirmed', { images, course_id: courseId });
//       const result = response.data.data;
//       if (!result.matched || !result.confirmed) return result;
//       return await this._markAttendance(result, courseId);
//     } catch (error) {
//       if (axios.isAxiosError(error)) throw new AppError(error.response?.data?.message || 'Face recognition failed', 502);
//       throw error;
...
//     if (!course) throw new AppError('Course not found', 404);
//     try {
//       const response = await biometricClient.post('/api/recognize/multi', { image: imageBase64, course_id: courseId });
//       const { faces_detected, matched_count, unmatched_count, matches } = response.data.data;
//       if (!matches || matches.length === 0) return { faces_detected, matched_count: 0, unmatched_count, results: [] };
//       const { AttendanceService } = await import('../../attendance/services/attendance.service');
//       const attendanceService = new AttendanceService();
//       const results = await Promise.all(
//         matches.map(async (match: any) => {
//           try {
//             const attendance = await attendanceService.mark({ student_id: match.student_id, course_id: courseId, distance_score: match.distance_score, verification_status: 'VERIFIED' });
//             return { ...match, already_marked: false, attendance };
//           } catch (err: any) {
//             if (err.message === 'Attendance already marked for today') return { ...match, already_marked: true };
//             return { ...match, error: err.message };
//           }
//         })
//       );
//       return { faces_detected, matched_count, unmatched_count, results };
//     } catch (error) {
//       if (axios.isAxiosError(error)) throw new AppError(error.response?.data?.message || 'Face recognition failed', 502);
//       throw error;
//     }
...
//       });
//       logger.info(`Recognized student ${result.student_id} for course ${courseId}`);
//       return { ...result, already_marked: false, attendance };
//     } catch (err: any) {
//       if (err.message === 'Attendance already marked for today') return { ...result, already_marked: true };
//       throw err;
//     }
//   }
...
//     const student = await prisma.student.findUnique({ where: { student_id: studentId } });
//     if (!student) throw new AppError('Student not found', 404);
//     const hasEmbedding = await prisma.faceEmbedding.findFirst({ where: { student_id: studentId } });
//     if (!hasEmbedding) throw new AppError('No face enrolled for this student', 400);
//     try {
//       const response = await biometricClient.post('/api/verify', { student_id: studentId, image: imageBase64 });
//       return response.data;
//     } catch (error) {
//       if (axios.isAxiosError(error)) throw new AppError(error.response?.data?.message || 'Face verification failed', 502);
//       throw error;
//     }
//   }

//   async deleteEmbedding(studentId: number) {
//     const embeddings = await prisma.faceEmbedding.findMany({ where: { student_id: studentId } });
//     if (!embeddings.length) throw new AppError('No embeddings found for this student', 404);
//     try { await biometricClient.delete(`/api/embeddings/${studentId}`); } catch { }
//     await prisma.faceEmbedding.deleteMany({ where: { student_id: studentId } });
//     return { message: 'Face embeddings deleted successfully' };
//   }
// }
